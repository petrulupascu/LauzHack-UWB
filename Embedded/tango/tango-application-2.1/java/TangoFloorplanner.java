...
public class TangoFloorplanner extends Tango.OnTangoUpdateListener {
   ...
   /**
    ...
    public interface OnFloorplanAvailableListener {
        void onFloorplanAvailable(List<TangoPolygon> polygons, List<TangoFloorplanLevel> levels);//complete in FloorPlanReconstructionActivity.java for use the data
    }
            ...
            mRunnableCallback = new Runnable() {
                ...
                public void run() {
                    ...
                    synchronized (TangoFloorplanner.this) {
                        ...
                        // Get the latest point cloud data.****the primary data****
                        ...
                        // Update the mesh and floorplan representation.****do some analyse****
                        mTango3dReconstruction.updateFloorplan(cloudData, depthPose);

                        // Extract the full set of floorplan polygons.****withdraw the data floorplan polygons*****
                        polygons = mTango3dReconstruction.extractFloorplan();

                        // Extract the full set of floorplan levels.****withdraw the data levels*****
                        levels = mTango3dReconstruction.extractFloorplanLevels();
                    }
                    // Provide the new floorplan polygons to the app via callback.*****provide the data here*******
